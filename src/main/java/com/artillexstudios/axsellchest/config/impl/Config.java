...
public class Config extends AbstractConfig {
    ...
    @Comment(" Can be RoseStacker, WildStacker, or default")
    public static String STACKER_INTEGRATION = "RoseStacker";

    @Key("integrations.economy")
    @Comment(" Can be Vault")
    public static String ECONOMY_INTEGRATION = "Vault";

    @Key("integrations.prices")
    @Comment(" Can be ShopGUIPlus or EconomyShopGUI, or zShop")
    public static String PRICES_INTEGRATION = "ShopGUIPlus";

    @Key("integrations.bank")
    @Comment(" Can be SuperiorSkyBlock or BentoBox ")
    public static String BANK_INTEGRATION = "SuperiorSkyBlock2";

    @Key("autosave-minutes")
    @Comment({" How often should we save data about chests?", " Note: this is only data like money made, items sold. Other changes are saved immediately. "})
    public static int AUTOSAVE_MINUTES = 1;

    @Key("default-chest-limit")
    @Comment({" This can be overwritten per player by giving them the", " axsellchest.limit.<amount> permission. "})
